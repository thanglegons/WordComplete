...
// If need to optimize memory, one should try to reduce this, then add bound-checks to the functions belows
    ...
    std::string VN_TONELESS_VOWELS = "aeiouy";
    std::string VN_LOWER_VOWELS = "aáàảãạâấầẩẫậăắằẳẵặeéèẻẽẹêếềểễệiíìỉĩịoóòỏõọôốồổỗộơớờởỡợuúùủũụưứừửữựyýỳỷỹỵ";
    ...
    int codepoint_to_tone[ALPHANUMERIC_SIZE];
    int codepoint_to_hat[ALPHANUMERIC_SIZE];
...
// Three frequently used functions, can remove bound-check if the codepoint is small for sure
    ...
    void init_tone_forms()
    {
        ...
        for (int i = 0; i < 24; ++i)
        {
            ...
            {
                if (*it != start) {
                    codepoint_to_tone[*it] = counter % 6;
                }
    ...
    bool is_vowel(const char& ch) {
        return std::any_of(VN_TONELESS_VOWELS.begin(), VN_TONELESS_VOWELS.end(), [ch](char vowel){return ch == vowel;});
    }

    bool is_valid_vn_word(const std::string& text) {
        std::string toneless_text = lower_root(text);
        int vowel_count = 0;
        for (char ch : toneless_text) {
            if (is_digit(ch)) return false;
            if (!is_alphabetic(ch)) return false;
            if (is_vowel(ch)) vowel_count ++;
        }
        if (vowel_count < toneless_text.size() && vowel_count > 0) return true;
        return false;
    }

    bool contain_digits(const std::string& text) {
        return std::any_of(text.begin(), text.end(), [](char c){return is_digit(c);});
    }
    ...
    std::string normalize_lower_NFD_UTF(const std::string& text) {
        std::string filtered_text = remove_special_chars(text);
        std::vector< uint32_t > codepoints = VnLangTool::to_lower_UTF(filtered_text);
        codepoints = VnLangTool::normalize_NFD_UTF(codepoints, true);
        return vector_to_string(codepoints);
    }

    std::vector<std::string> generate_ngram(const std::string& text, int n) {
        std::vector<std::string> tokens = split_str(text, " ");
        int token_len = tokens.size();
        std::vector<std::string> ngrams;
        for (int i = 0; i < token_len - n + 1; i++) {
            std::string ngram;
            for (int j = 1; j <= n; j++) {
                if (tokens[i + j - 1] == " ") continue;
                ngram += tokens[i + j - 1];
                if (j < n) ngram += " ";
            }
